import { SistemaContratosData, Contrato } from '../../domain/interfaces/contrato.interface';
import fs from 'fs/promises';
import path from 'path';
import { DateUtils } from '../utils/date.utils';

export class JsonStorage {
  private data: SistemaContratosData;
  private readonly filePath: string;
  private cache: SistemaContratosData | null = null;
  ...
  private getEstructuraInicial(): SistemaContratosData {
    return {
      honorarios: {},
      contrata: {},
      planta: {},
  ...
  async guardar(): Promise<void> {
    ...
    this.data.metadata.total_contratos = 
  ...
  obtenerData(): SistemaContratosData {
    return { ...this.data };
  }

  actualizarData(data: SistemaContratosData): void {
    this.data = data;
  }

  // Métodos de conveniencia
  obtenerContratos() {
    // Retorna objeto combinado de todos los contratos
    return {
  ...
  // Guardar contrato en la sección correspondiente según su tipo
  guardarContrato(id: string, contrato: any): void {
    const tipo = contrato.tipo;
    if (tipo === 'honorarios') {
      this.data.honorarios[id] = contrato;
    } else if (tipo === 'contrata') {
      this.data.contrata[id] = contrato;
    } else if (tipo === 'planta') {
      this.data.planta[id] = contrato;
    } else {
      throw new Error(`Tipo de contrato no soportado: ${tipo}`);
    }
  }

  // Eliminar contrato de la sección correspondiente
  eliminarContrato(id: string): boolean {
  ...
  // Obtener contrato por ID (busca en todas las secciones)
  obtenerContratoPorId(id: string): Contrato | null {
