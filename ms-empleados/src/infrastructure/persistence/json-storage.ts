import { SistemaEmpleadosData } from '../../domain/interfaces/empleado.interface';
import fs from 'fs/promises';
import path from 'path';
import { DateUtils } from '../utils/date.utils';

export class JsonStorage {
  private data: SistemaEmpleadosData;
  private readonly filePath: string;
  private cache: SistemaEmpleadosData | null = null;
  ...
  private getEstructuraInicial(): SistemaEmpleadosData {
    return {
      empleados: {},
      metadata: {
  ...
  async cargar(force: boolean = false): Promise<void> {
    ...
    try {
      ...
      if (!loadedData.empleados || !loadedData.metadata) {
  ...
  async guardar(): Promise<void> {
    ...
    this.data.metadata.total_empleados = Object.keys(this.data.empleados).length;
  ...
  obtenerData(): SistemaEmpleadosData {
    return { ...this.data };
  }

  actualizarData(data: SistemaEmpleadosData): void {
    this.data = data;
  }
